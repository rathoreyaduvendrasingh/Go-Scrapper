...
func main(){
	res, err:= GoogleScrape("Yaduvendra Singh","com",1)
	if err == nil{
		for _,res := range res{
			fmt.Println(res)
		}
...
func buildGoogleUrls(searchTerm string,countryCode string, pages int)([]string,error){
	toScrape := []string{}
	...
	if googleBase, found := googleDomains[countryCode] ; found{
		for i := 0;i<pages;i++{
			start := i*30
			scrapeURL := fmt.Sprintf("%s%s&num=%d&hl=%s&start=%d&filter=0",googleBase, searchTerm, 30,"en", start)
			toScrape = append(toScrape, scrapeURL)
		}
	}else{
		err:=fmt.Errorf("country code %s not supported",countryCode)
		return nil,err
	}
	return toScrape,nil
}
...
func scrapeClientRequest(searchURL string,proxyString interface{}) (*http.Response,error){
	...
	if res.StatusCode != 200 {
		err := fmt.Errorf("scraper recieved a non-200 status")
		return nil,err
	}
...
func GoogleScrape(s string,countryCode string,pages int) ([]SearchResult, error){
	results := []SearchResult{}
	resultCounter := 0
	googlePages,err := buildGoogleUrls(s,countryCode,pages) 
	if err != nil{
		return nil,err
	}
